...
Constants.WEAVE_WINDOW = 0.7     -- Reduced from 1.0 to 0.7 for better oGCD weaving
Constants.HEALING_LOCKOUT = 0.8   -- Time in seconds to prevent healing spell spam
Constants.MIN_SPELL_SPACING = 0.5 -- Minimum time between spell casts
Constants.OGCD_WINDOW_START = 0.3 -- Start of oGCD weaving window after GCD
Constants.OGCD_WINDOW_END = 0.7   -- End of oGCD weaving window before next GCD

-- Initialize and expose the module
Olympus = Olympus or {}
Olympus.Core = Olympus.Core or {}
Olympus.Core.Constants = Constants

-- Log initial configuration
Debug.Info(Debug.CATEGORIES.SYSTEM, "Olympus.Core.Constants initialized")
Debug.Verbose(Constants.DEBUG_CATEGORIES.TIMING, 
    string.format("Weaving windows configured - Start: %.1fs, End: %.1fs", 
        Constants.OGCD_WINDOW_START,
        Constants.OGCD_WINDOW_END))
